...
for i in {1..10}; do
  PENDING=$(gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR_NUMBER}/requested_reviewers \
    --jq '[.users[] | select(.login | test("copilot"; "i"))] | length')
  if [ "$PENDING" -eq 0 ]; then
    # Verify Copilot actually submitted a review (not just never assigned)
    REVIEWED=$(gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR_NUMBER}/reviews \
...
```bash
gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR_NUMBER}/comments \
...
```bash
# Fixed
gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR_NUMBER}/comments/{COMMENT_ID}/replies \
...
REVIEWS_BEFORE=$(gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR_NUMBER}/reviews \
  --jq '[.[] | select(.user.login | test("copilot"; "i"))] | length')
COMMENTS_BEFORE=$(gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR_NUMBER}/comments \
...
```bash
gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR_NUMBER}/requested_reviewers \
...
for i in {1..10}; do
  ...
  if [ "$PENDING" -eq 0 ]; then
    ...
    REVIEWS_AFTER=$(gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR_NUMBER}/reviews \
      --jq '[.[] | select(.user.login | test("copilot"; "i"))] | length')
...
- "Intentional — see CLAUDE.md for design rationale"

## GitHub API Reference

```bash
# List PR inline comments (review comments on diff)
gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR}/comments

# Reply to a specific comment (in its thread)
gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR}/comments/{ID}/replies -f body="..."

# List PR reviews (approve/request changes)
gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR}/reviews
...
gh api repos/seantokuzo/seantokuzo-dot-dev/pulls/{PR}/requested_reviewers
