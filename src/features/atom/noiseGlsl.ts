/**
 * Compact 3D simplex noise for GLSL vertex displacement.
 * Shared between Nucleus (mouse-reactive) and ProjectOrb (hover-reactive).
 */
export const SIMPLEX_NOISE_3D = /* glsl */ `
  vec3 mod289v(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
  vec4 mod289v4(vec4 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
  vec4 permutev(vec4 x) { return mod289v4(((x * 34.0) + 10.0) * x); }
